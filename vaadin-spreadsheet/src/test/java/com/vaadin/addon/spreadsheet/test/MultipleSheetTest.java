...
public class MultipleSheetTest extends AbstractSpreadsheetTestCase {
    ...
    public void testMultipleSheet() {
        ...
        driver.findElement(By.xpath("//div[contains(@class,'col4 row4')]"))
                .click();
        testBenchElement(driver
                .findElement(By.xpath("//div[contains(@class, 'col6 row6')]")))
                        .click(8, 8, Keys.CONTROL);
        testBench(driver).waitForVaadin();

        spreadsheet.getCellAt("B2").setValue("sheet0 value");
        assertEquals("sheet0 value", sheetController.getCellContent("B2"));

        spreadsheet.addSheet();
        spreadsheet.selectSheet("new sheet");

        assertEquals("", spreadsheet.getCellAt("B2").getValue());
        spreadsheet.getCellAt("B2").setValue("sheet1 value");
        spreadsheet.selectSheet("Sheet1");
        testBench(driver).waitForVaadin();
        assertEquals("", spreadsheet.getCellAt("C3").getValue());
        assertEquals("sheet0 value", spreadsheet.getCellAt("B2").getValue());
    }
