...
public class VaadinBrowserFactory extends DefaultBrowserFactory {
    ...
    @Override public DesiredCapabilities create(Browser browser) {
        switch (browser) {
        case IE8:
    ...
    private DesiredCapabilities createIE(Browser browser, String version) {
        DesiredCapabilities capabilities = create(browser, version,
            Platform.WINDOWS);
        capabilities
            .setCapability(InternetExplorerDriver.IE_ENSURE_CLEAN_SESSION,
                true);
        return capabilities;
    }

    private DesiredCapabilities createFirefox() {
        DesiredCapabilities desiredCapabilities = create(Browser.FIREFOX,
